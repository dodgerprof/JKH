<template>
  <div class="page-wrapper page-wrapper--auth">
    <div class="col-xs-12 col-sm-8 col-md-5">
      <div class="panel panel--center-item">
        ...
        <form class="form">
          ...
          <div class="form-group">
            ...
            <input v-model='repeatPassword' id="confirm_password" type="password" placeholder="Подтвердите пароль" :class="[{'is-invalid': $v.repeatPassword.$error}, 'form-control']" @input="$v.repeatPassword.$touch()">
          </div>
          <div v-if="$v.email.$error || $v.password.$error || $v.repeatPassword.$error || submitStatus" class="alert alert-danger">
            ...
            <div v-if="$v.repeatPassword.$error">Пароли должны совпадать</div>
            <div v-if="submitStatus">{{submitStatus}}</div>
          </div>
          <div class="mb-3">
            <button type="submit" class="btn btn-primary btn-block" @click.prevent="onSubmit" :disabled="loading">Регистрация
              <span v-if="loading" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
            </button>
          </div>
          <div class="d-flex align-items-center justify-content-center">
            <p class="no-account">Already have an account? You need to
              <router-link tag="a" to="/">log in</router-link>
            </p>
          </div>
        </form>
      </div>
      ...
      onSubmit() {
        ...
        if (!this.$v.$invalid) {
          ...
          this.$store.dispatch('registerUser', user)
            ...
            .catch((error) => {
              this.submitStatus = error.response.data.error.message;
            })
        }
      }
    }
